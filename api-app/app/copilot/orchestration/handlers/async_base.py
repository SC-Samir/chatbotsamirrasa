"""
Async base classes for command handlers.

This module provides the foundation for all async command handlers in the
orchestration engine, including the base handler class and decorator
for registering async handlers.
"""
...
# Async handler type alias
AsyncHandler = Callable[[CommandRequest], Any]


@dataclass(frozen=True)
class AsyncHandlerConfig:
    """
    Configuration for an async command handler.
...
class AsyncHandlerRegistry:
    """
    Registry of all async command handlers.
    
    This class maintains a mapping from command names to their
    corresponding async handler functions and metadata.
    """
    handlers: Dict[str, AsyncHandler] = field(default_factory=dict)
    metadata: Dict[str, AsyncHandlerConfig] = field(default_factory=dict)
    
    def register(
        self,
        command_name: str,
        handler: AsyncHandler,
        config: Optional[AsyncHandlerConfig] = None,
    ) -> None:
    ...
    def get_handler(self, command_name: str) -> Optional[AsyncHandler]:
        """Get the async handler for a command."""
        return self.handlers.get(command_name)
    
    def get_metadata(self, command_name: str) -> Optional[AsyncHandlerConfig]:
...
class AsyncBaseCommandHandler:
    """
    Async base class for all async command handlers.
    ...
    async def _with_mutation_preview(
        self,
        request: CommandRequest,
...
# Protocol for async handler classes
class AsyncCommandHandler:
    """
    Protocol for async command handler classes.
    
    All async command handlers should implement the 'handle_async' method
    that takes a CommandRequest and returns a CommandResult.
    """
    
    async def handle_async(self, request: CommandRequest) -> CommandResult:
        """
        Handle a command request asynchronously.
...
def async_handler(
    ...
    """
    Decorator to register an async handler class with a command name.
    
    This decorator can be used to automatically register handler
    classes with the async command engine.
    ...
    Usage:
        @async_handler("apps.list", AsyncHandlerConfig(required_entities=("region",)))
        class AsyncAppListHandler(AsyncBaseCommandHandler):
            async def handle_async(self, request: CommandRequest) -> CommandResult:
                # implementation
                pass
    """
    def decorator(cls: type) -> type:
        ...
        setattr(cls, "_async_command_name", command_name)
        setattr(cls, "_async_handler_config", config or AsyncHandlerConfig())
        return cls
    return decorator
